...
class _ECGChartScreenState extends State<ECGChartScreen> {
  ...
  bool isValidECG(double ecg) => ecg >= 30 && ecg <= 180;
  ...
  List<Map<String, dynamic>> getFilteredData() {
    ...
    return trendData.where((e) {
      final ecg= double.tryParse(e['bpm']?.toString().trim() ?? '') ?? 0.0;
      final time = DateTime.tryParse(e['timestamp'] ?? '');
      return isValidECG(ecg) && time != null && time.isAfter(cutoff);
    }).toList();
  }
                    ...
                    Center(
                      child: Column(
                        children: const [
                          ...
                          Text("ECG", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.black54)),
                        ],
                      ),
                    ),
  ...
  Widget _buildGradientChart() {
    ...
    if (filteredTrend.isEmpty) return const Center(child: Text("No ECG data."));

    final spots = <FlSpot>[];
    final dateTimes = <DateTime>[];
    final ecgs = <double>[]; // <-- ADD THIS

    for (int i = 0; i < filteredTrend.length; i++) {
      final ecg = double.tryParse(filteredTrend[i]['bpm'])!;
      final time = DateTime.parse(filteredTrend[i]['timestamp']);
      ecgs.add(ecg); // <-- ADD THIS
      spots.add(FlSpot(i.toDouble(), ecg));
      dateTimes.add(time);
    }
    ...
    return Container(
      ...
      child: LineChart(
        LineChartData(
          minY: ecgs.reduce(min) - 10,
          maxY: ecgs.reduce(max) + 10,
          ...
          lineBarsData: [
            LineChartBarData(
              ...
              getDotPainter: (spot, percent, barData, index) {
                final status = (filteredTrend[index]['ecgstatus'] ?? "").toString().trim().toLowerCase();
  ...
  final ecgs = filtered
      .map((e) => double.tryParse(e['bpm'] ?? '') ?? 0.0)
      .where((ecg) => isValidECG(ecg))
      .toList();
  ...
  final min = ecgs.reduce((a, b) => a < b ? a : b);
  final max = ecgs.reduce((a, b) => a > b ? a : b);
  final avg = ecgs.reduce((a, b) => a + b) / ecgs.length;
  ...
  if (filtered.isEmpty) {
    return const Center(
      child: Padding(
        ...
        child: Text("No ECG data available."),
      ),
    );
  }
  ...
  if (selectedTime.toLowerCase() == "24h") {
    ...
    for (final entry in filtered.reversed) {
      ...
      if (!shownLabels.contains(label)) {
        ...
        rows.add({
          "time": label,
          "ecg": "${entry['bpm']} BPM",
          "status": entry['ecgstatus'] ?? "-",
        });
      }
    }
  } else {
    // Group by label (day/month), then average ecgand majority status
    ...
    grouped.forEach((label, entries) {
      ...
      final avgEcg=
          ecgs.reduce((a, b) => a + b) / ecgs.length;

      final statusCounts = <String, int>{};
      for (final e in entries) {
        final status = e['ecgstatus'] ?? "-";
      ...
      rows.add({
        "time": label,
        "ecg": "${avgEcg.toStringAsFixed(1)} F",
        "status": topStatus,
  ...
  return Container(
    ...
    child: Column(
      children: [
        ...
        ...rows.map((e) => Column(
              children: [
                _tableRow(e["time"]!, e["ecg"]!, e["status"]!),
                Divider(color: Colors.grey.shade300),
              ],
            )),
      ],
...
Widget _tableRow(String time, String reading, String status) {
  final abnormalStatuses = ["low", "high"];
